...
| Shared-mailbox **read** works but **move/categorize/flag** fails with `404 ErrorInvalidMailboxItemId` or "folder not found" | The write tool must target the shared mailbox, and the token must carry `Mail.ReadWrite.Shared`. (1) Pass `sharedMailbox` (alias `email`) on the write tool — `folders action=move`, `apply-category`, `update-email`. (2) Add the delegated `Mail.ReadWrite.Shared` permission to your Azure app and grant consent. (3) **Re-authenticate** so the refreshed token includes the new scope. Without these, writes fall back to your own mailbox, where the shared message ID doesn't exist. |
